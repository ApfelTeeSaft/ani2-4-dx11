...
#endif // STARTUPANIMATION
{
    ...
    XAudio2Create( NULL, &pDSound, NULL );
#endif
...
void XBoxStartupApp::advanceTime(float dt,float cam_dt)
{
	...
	else
	{
		...
		if( XINPUT_STATE[XINPUT_GAMEPAD_LEFT_SHOULDER])
		{
			float fval = ((float)XINPUT_STATE[XINPUT_GAMEPAD_LEFT_SHOULDER]) / 255.0f;
			fBlobIntensity -= (f_int_vel * cam_dt * fval);
			fBlobIntensity = max(0.f,fBlobIntensity);
		}

		if( XINPUT_STATE[XINPUT_GAMEPAD_RIGHT_SHOULDER])
		{
			float fval = ((float)XINPUT_STATE[XINPUT_GAMEPAD_RIGHT_SHOULDER]) / 255.0f;
			fBlobIntensity += (f_int_vel * cam_dt * fval);
	...
	// idk what this means, and why would it trace the color of the controller?
